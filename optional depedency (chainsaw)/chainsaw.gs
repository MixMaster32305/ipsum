...
ChainSaw.optimize_chains = false // potential optimzation by sorting trigrams by frequency
...
ChainSaw.init = function // Initialize ChainSaw
	...
	if ChainSaw.optimize_chains then
		ChainSaw.Log("DEBUG", "Optimizing chains by frequency...")
		ChainSaw.sort_chains_by_frequency
	end if
...
ChainSaw.calculate_trigram_frequencies = function()
    // Calculate how often each trigram appears in samples
...
ChainSaw.sort_chains_by_frequency = function()
    // Sort chain indexes by frequency (most common first)
    ChainSaw.Log("DEBUG", "Sorting chains by frequency...")

    if ChainSaw.trigram_frequencies == null then
        ChainSaw.calculate_trigram_frequencies
    end if
    ...
    // Replace chain indexes with sorted version
    sorted_indexes = []
...
ChainSaw.parse_params = function(param_list) // parse command-line arguments into ChainSaw params
	...
	for param in param_list
		...
		if key == "--loglevel" then
			...
			if value.lower == "true" or value.lower == "1" then
				ChainSaw.optimize_chains = true
				ChainSaw.Log("DEBUG", "Optimize chains by frequency: " + ChainSaw.optimize_chains)
			end if
...
print_help = function // Print the available commands
	...
	[run/load]	--optimize_chains/ChainSaw.optimize_chains	false	Optimize chains by frequency. May speed up cracking.
...
ChainSaw.unittest_parse_params = function // Unit test for the parse_params function
	...
	params = "unittest --user=test_user --ip=1.1.1.1 --port=99 --min_length=1 --max_length=10 --order=1 --loglevel=DEBUG --test_pass=test_pass --optimize_chains=true"
	...
	if ChainSaw.user != "test_user" then
		...
		return
	else if globals.testing_pass != "test_pass" then
		ChainSaw.Log("ERROR", "Test password " + globals.testing_pass + " != test_pass")
		return
	else if ChainSaw.optimize_chains != true then
		ChainSaw.Log("ERROR", "Optimize chains " + ChainSaw.optimize_chains.to_string + " != true")
		return
	end if
